...
for (i, sample) in enumerate(samples)
    ...
    font = "Avenir Book"
    fontcolor = "#474747"
    Label(fig[1, 1, Top()], halign = :left, L"\times 10^{-3}", fontsize = 16,
        color = fontcolor, font = font)
    ax = Axis(fig[1, 1], xlabel = "Time [days]", ylabel = "Concentration [mol L⁻¹]",
    title = "Zero-order late time behavior",
    titlefont = font,
    ...
    for (j, meas_name) in enumerate(meas_names)
        ...
        #values = values.*1e-3
        if j == 1
            lines!(ax, df[!,:t], model_no3.(df[!,:t]).*1e3, label = meas_name, color = colors[j], linestyle = :dash,
                    linewidth = 2.8)
            if t0 > 0
                tfill = 0:0.1:t_of_zero
                fill_between!(ax, tfill, model_no3.(tfill).*1e3,
                    data_fit.(tfill).*1e3, color = colors[j], alpha = 0.5)
                
            end
        elseif j == 2
            lines!(ax, df[!,:t], repeat([doc_ss], length(df[!,:t])).*1e3, label = meas_name, color = colors[j], linestyle = :dash,
                linewidth = 2)
        end
    ...
    text!(ax, 0.5, 0.9,
        ...
        color = fontcolor,
        fontsize = 16, space = :relative)

    axislegend(ax, position = :rt, merge = true, 
        titlefont = font, titlesize = 18, titlecolor = fontcolor,
        labelsize = 16, labelcolor = fontcolor, backgroundcolor = :transparent)
    fig
    save(plotsdir("$(sample)_integral_vjoh.png"), fig)
    save(plotsdir("$(sample)_integral_vjoh.svg"), fig)
